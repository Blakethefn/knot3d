...
def test_full_trefoil_analyze_from_ui(real_gui_window, wait_until):
    ...
    wait_until(lambda: real_gui_window.session_state.analysis_result is not None, timeout_ms=10000)
...
def test_full_trefoil_unknotting_from_ui(real_gui_window, wait_until):
    ...
    wait_until(lambda: real_gui_window.session_state.unknotting_result is not None, timeout_ms=15000)
    assert real_gui_window.session_state.unknotting_result["upper_bound"] == 1


def test_select_candidate_updates_views(real_gui_window, wait_until):
    real_gui_window.example_picker.select_example("trefoil")
    real_gui_window.run_panel.unknot_button.click()
    wait_until(lambda: real_gui_window.crossing_table.model.rowCount() > 0, timeout_ms=15000)
    real_gui_window.crossing_table.select_row(0)
    wait_until(lambda: real_gui_window.session_state.selected_crossing_index == 0, timeout_ms=5000)
    assert "Candidate" in real_gui_window.comparison_view.meta_label.text()
